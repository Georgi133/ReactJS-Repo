...
export default function UserList({
...
})
 {
    ...
    const [showDeleteUserInfo ,setShowDeleteUserInfo] = useState(null);
    ...
    const onUserUpdateSubmitHandler = (e, userId) => {
        onUserUpdate(e, userId);
        setUserEdit(null);
    }
    

    const editUser = async (userId) => {
        const userEdit = await userService.getOne(userId)
    ...
    const onDeleteClick = (userId) => {
        setShowDeleteUserInfo(userId);
        // const response = userService.deleteIt(userId);
    };

    const closeDeleteList = () => {
        setShowDeleteUserInfo(null);
    };

    const onDeleteHandler = () => {
        onUserDelete(showDeleteUserInfo);
        closeDeleteList();
    };
    ...
    return (
        <>
            {userEdit && <UserUpdating user={userEdit} onCloseEdit={onCloseEdit} onUserUpdate={onUserUpdateSubmitHandler}/>}
            {selectedUser && <UserDetails {...selectedUser} onClose={onClose}/>}
            {showDeleteUserInfo && <DeleteUser closeDeleteList={closeDeleteList} onUserDelete={onDeleteHandler}/>}
